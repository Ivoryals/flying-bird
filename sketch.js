...
// One visual weight for dividers, selection capsules, active circles and buttons.
const PAGMAR_INTERFACE_STROKE = 0.5;

// One fixed footer action slot across every project screen.
// HINT and GRID always occupy the same dimensions and the same position.
const PAGMAR_FOOTER_ACTION_WIDTH = 140;
const PAGMAR_FOOTER_ACTION_HEIGHT = 28;
const PAGMAR_FOOTER_PAGE_SLOT_WIDTH = 142;
const PAGMAR_FOOTER_ACTION_GAP = 16;
...
let pagmarInteractionGuideMode = "";
let gridInteractionGuideDismissed = false;
let borderInteractionGuideDismissed = false;
let memoryInteractionGuideDismissed = false;
let belongingInteractionGuideDismissed = false;
let reflectionInteractionGuideDismissed = false;

const PAGMAR_GUIDE_INACTIVITY_MS = 3000;
let borderInteractionGuideLastActivityAt = -Infinity;
let memoryInteractionGuideLastActivityAt = -Infinity;
let belongingInteractionGuideLastActivityAt = -Infinity;
let reflectionInteractionGuideLastActivityAt = -Infinity;
let homeInteractionGuideRestartTimer = null;
let homeInteractionGuideActivityHandler = null;

function getPagmarGuideClock() {
  if (typeof millis === "function") return millis();
  if (typeof performance !== "undefined") return performance.now();
  return Date.now();
}

function markPagmarInteractionGuideActivity(screenName) {
  const now = getPagmarGuideClock();

  if (screenName === "border") {
    borderInteractionGuideDismissed = true;
    borderInteractionGuideLastActivityAt = now;
  } else if (screenName === "memory") {
    memoryInteractionGuideDismissed = true;
    memoryInteractionGuideLastActivityAt = now;
  } else if (screenName === "belonging") {
    belongingInteractionGuideDismissed = true;
    belongingInteractionGuideLastActivityAt = now;
  } else if (screenName === "reflection") {
    reflectionInteractionGuideDismissed = true;
    reflectionInteractionGuideLastActivityAt = now;
  }

  updatePagmarInteractionGuide();
}

function markActivePagmarWordScreenActivity(event) {
  // Pointer movement only counts while the visitor is actively dragging.
  if (
    event &&
  ...
  if (
    typeof reflectionTicketOpen !== "undefined" &&
    reflectionTicketOpen
  ) {
    markPagmarInteractionGuideActivity("reflection");
    return;
  }

  if (borderPopupOpen) {
    markPagmarInteractionGuideActivity("border");
    return;
  }

  if (memoryPopupOpen) {
    markPagmarInteractionGuideActivity("memory");
    return;
  }
  ...
  if (
    homePopupOpen &&
    typeof homeInteractionGuideActivityHandler === "function"
  ) {
    homeInteractionGuideActivityHandler();
  }
}

function restorePagmarInteractionGuideAfterInactivity() {
  const now = getPagmarGuideClock();

  if (
    borderInteractionGuideDismissed &&
    now - borderInteractionGuideLastActivityAt >=
      PAGMAR_GUIDE_INACTIVITY_MS
  ) {
    borderInteractionGuideDismissed = false;
  }

  if (
    memoryInteractionGuideDismissed &&
    now - memoryInteractionGuideLastActivityAt >=
      PAGMAR_GUIDE_INACTIVITY_MS
  ) {
    memoryInteractionGuideDismissed = false;
  }

  if (
    belongingInteractionGuideDismissed &&
    now - belongingInteractionGuideLastActivityAt >=
      PAGMAR_GUIDE_INACTIVITY_MS
  ) {
    belongingInteractionGuideDismissed = false;
  }

  if (
    reflectionInteractionGuideDismissed &&
    now - reflectionInteractionGuideLastActivityAt >=
      PAGMAR_GUIDE_INACTIVITY_MS
  ) {
    reflectionInteractionGuideDismissed = false;
  }
}

function setupPagmarInteractionGuide() {
  if (typeof document === "undefined" || pagmarInteractionGuide) return;

  const style = document.createElement("style");
  style.id = "pagmar-interaction-guide-style";
  style.textContent = `
    #pagmar-interaction-guide {
      position: fixed;
      left: 50%;
      bottom: 74px;
      z-index: 1000002;
    ...
    #pagmar-interaction-guide.is-visible {
      opacity: 1;
      transform: translate(-50%, 0);
    }

    #pagmar-interaction-guide.is-dark {
      color: #ffffff;
    }
    ...
    .pagmar-guide-icon-scroll::before,
    .pagmar-guide-icon-scroll::after {
      content: "";
      position: absolute;
      left: 50%;
      width: 13px;
      ...
      animation: pagmar-guide-scroll 1.35s ease-in-out infinite;
    }
    ...
    /* Main grid: small letter marks with a selection capsule expanding
       from right to left. This represents selecting a word, not a slider. */
    .pagmar-guide-icon-drag {
      width: 72px;
      height: 26px;
    }

    .pagmar-guide-icon-drag::before {
      content: "";
      position: absolute;
      left: 8px;
      top: 9px;
      ...
      box-shadow:
        ...
        50px 0 0 currentColor;
    }

    .pagmar-guide-icon-drag::after {
      content: "";
      position: absolute;
      right: 3px;
      top: 3px;
      ...
      border-radius: 999px;
      animation: pagmar-guide-word-select 1.7s ease-in-out infinite;
    }

    /* Reflection only: this remains a true slider animation. */
    .pagmar-guide-icon-slider::before {
      content: "";
      position: absolute;
      left: 4px;
      right: 4px;
    ...
    .pagmar-guide-icon-slider::after {
      content: "";
      position: absolute;
      top: 50%;
      left: 5px;
      ...
      border-radius: 50%;
      background: currentColor;
      animation: pagmar-guide-horizontal 2.45s ease-in-out infinite;
    }

    /* Memory: two moving curved lines suggest air/wind, not sliding. */
    .pagmar-guide-icon-breeze::before,
    .pagmar-guide-icon-breeze::after {
      content: "";
      position: absolute;
      left: 5px;
      width: 38px;
      ...
      border-radius: 50%;
      animation: pagmar-guide-breeze 1.8s ease-in-out infinite;
    }
    ...
    .pagmar-guide-icon-lens::before {
      ...
      animation: pagmar-guide-lens 1.55s ease-in-out infinite;
    }

    .pagmar-guide-icon-lens::after {
      content: "";
      position: absolute;
      left: 50%;
      top: 50%;
    ...
    @keyframes pagmar-guide-scroll {
      0%, 100% { opacity: .18; transform: translate(-50%, -3px) rotate(45deg); }
      50% { opacity: 1; transform: translate(-50%, 4px) rotate(45deg); }
    }

    @keyframes pagmar-guide-word-select {
      0%, 100% {
        width: 12px;
        opacity: .24;
      }
    ...
    @keyframes pagmar-guide-horizontal {
      0%, 100% { left: 5px; opacity: .3; }
      50% { left: 35px; opacity: 1; }
    }

    @keyframes pagmar-guide-breeze {
      0%, 100% {
    ...
    @keyframes pagmar-guide-lens {
      0%, 100% { transform: translate(-50%, -50%) scale(.84); opacity: .35; }
      50% { transform: translate(-50%, -50%) scale(1.12); opacity: 1; }
    }
  `;
  document.head.appendChild(style);

  pagmarInteractionGuide = document.createElement("div");
  pagmarInteractionGuide.id = "pagmar-interaction-guide";
  pagmarInteractionGuide.setAttribute("aria-hidden", "true");
  pagmarInteractionGuide.innerHTML = `
    <div class="pagmar-guide-icon"></div>
  `;
  document.body.appendChild(pagmarInteractionGuide);

  // Any real interaction inside an open word screen hides its cue
  // immediately. Continued interaction resets the 3-second timer.
  [
    "pointerdown",
    "pointermove",
    "touchstart",
    ...
    document.addEventListener(
      eventName,
      markActivePagmarWordScreenActivity,
      { capture: true, passive: true }
    );
  });
}

function showPagmarInteractionGuide(mode, dark) {
  setupPagmarInteractionGuide();
  if (!pagmarInteractionGuide) return;

  const icon = pagmarInteractionGuide.querySelector(".pagmar-guide-icon");

  if (pagmarInteractionGuideMode !== mode && icon) {
    icon.className = "pagmar-guide-icon pagmar-guide-icon-" + mode;
    pagmarInteractionGuideMode = mode;
  }

  pagmarInteractionGuide.style.bottom =
    Math.round(getFooterHeight() + 15) + "px";
  pagmarInteractionGuide.classList.toggle("is-dark", Boolean(dark));
  pagmarInteractionGuide.classList.add("is-visible");
  pagmarInteractionGuide.setAttribute("aria-hidden", "false");
}

function hidePagmarInteractionGuide() {
  if (!pagmarInteractionGuide) return;
  pagmarInteractionGuide.classList.remove("is-visible");
  pagmarInteractionGuide.setAttribute("aria-hidden", "true");
}

function updatePagmarInteractionGuide() {
  if (typeof document === "undefined") return;

  restorePagmarInteractionGuideAfterInactivity();

  const birdScreen = document.getElementById("pagmar-bird-screen");
  if (birdScreen && birdScreen.style.display !== "none") {
    hidePagmarInteractionGuide();
    return;
  }

  if (reflectionTicketOpen) {
    if (reflectionInteractionGuideDismissed) {
      hidePagmarInteractionGuide();
    } else {
      showPagmarInteractionGuide("slider", false);
    }
    return;
  }

  if (borderPopupOpen) {
    if (borderInteractionGuideDismissed) {
      hidePagmarInteractionGuide();
    } else {
      showPagmarInteractionGuide("scroll", true);
    }
    return;
  }

  if (memoryPopupOpen) {
    if (memoryInteractionGuideDismissed) {
      hidePagmarInteractionGuide();
    } else {
      showPagmarInteractionGuide("breeze", false);
    }
    return;
  }

  if (belongingPopupOpen) {
    if (belongingInteractionGuideDismissed) {
      hidePagmarInteractionGuide();
    } else {
      showPagmarInteractionGuide("lens", false);
    }
    return;
  }
...
// A newly found word remains on the grid briefly before its interface opens.
// Its outline is already drawn by the visitor's drag, so it does not replay
// the centre-to-capsule animation at the moment of discovery.
let pendingFoundWordAnimation = null;
const FOUND_WORD_SCREEN_OPEN_DELAY = 900;

// The centre-to-capsule animation is reserved only for returning from a word
// interface to the grid. The letters enter first, then found outlines reform.
let gridReturnCircleRevealActive = false;
let gridReturnCircleRevealStartTime = -1;
const GRID_RETURN_CIRCLE_REVEAL_DURATION = 820;
const GRID_RETURN_CIRCLE_REVEAL_STAGGER = 95;
const PAGMAR_SELECTION_STROKE_ALPHA = 0.46;
...
// 1) slowly pulse the row, 2) smoothly flash the first letter.
// Repeated presses during stage 2 replay the same first-letter flash.
let footerHintRect = null;
let footerGridRect = null;
let gridHintStage = 0;
let gridHintPair = null;
let gridHintFlashStartedAt = -Infinity;
const GRID_HINT_FLASH_CYCLE_MS = 1500;
const GRID_HINT_FLASH_CYCLES = 3;
...
let borderScreenBoard = null;
let borderScreenTrailSvg = null;
let borderScreenImageHotspot = null;
let borderScreenTextHotspot = null;
let borderScreenSoundHotspot = null;
let borderScreenSoundTriggered = false;
let borderTextScrollProgress = 0;
let borderTrailProgress = 0;
let borderTrailTargetProgress = 0;
let borderTrailAnimationFrame = null;
...
let belongingHorizontalMaxOffset = 0;
let belongingHorizontalDragging = false;
let belongingHorizontalDragStartX = 0;
let belongingHorizontalDragStartOffset = 0;
...
function draw() {
  ...
  updatePendingFoundWordAnimation();
  updatePagmarInteractionGuide();
...
function restartGridEntryAnimation() {
  ...
  gridReturnCircleRevealActive = foundWords.length > 0;
  gridReturnCircleRevealStartTime = -1;
  currentSelection = [];
...
function beginLanguageMorphSettling() {
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      let cell = languageMorphGrid[r] && languageMorphGrid[r][c];
      if (!cell) continue;

      freezeWaitingLanguageMorphState(cell.hebrew);
      freezeWaitingLanguageMorphState(cell.arabic);
    }
  }
}

function freezeWaitingLanguageMorphState(state) {
  if (!state) return;

  // Waiting letters stop scheduling new changes. Letters already halfway
  // through a shuffle finish that one change before the landing begins.
  if (state.waiting && !state.transitioning) {
    state.transitionStart = Infinity;
  }
}

function updateLanguageMorphSettling(now) {
  let allSettled = true;
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      if (!settleLanguageMorphState(cell.hebrew, now)) {
        allSettled = false;
      }

      if (!settleLanguageMorphState(cell.arabic, now)) {
        allSettled = false;
      }
    }
  }
...
function settleLanguageMorphState(state, now) {
  if (!state || !state.transitioning) return true;

  let endTime = state.transitionStart + state.transitionDuration;
    ...
    if (animatingGridChange) {
      ...
      for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
          ...
          revealAlpha *= getGridHintCellAlphaMultiplier(r, c);
      ...
      if (returnRevealFinished) {
        if (gridReturnRevealActive) {
          ...
          if (gridReturnCircleRevealActive && foundWords.length > 0) {
            gridReturnCircleRevealStartTime = millis();
          }
        }
...
function drawFoundWordCircles(alphaVal) {
  ...
  let returnRevealFinished = true;

  for (let i = 0; i < wordsToDraw.length; i++) {
    let fw = wordsToDraw[i];
    if (!fw || !fw.cells || fw.cells.length === 0) continue;

    let revealProgress = drawFoundWordCircle(fw, alphaVal, i);

    if (
      gridReturnCircleRevealActive &&
      gridReturnCircleRevealStartTime >= 0 &&
      revealProgress < 1
    ) {
      returnRevealFinished = false;
    }
  }
  ...
  if (
    gridReturnCircleRevealActive &&
    gridReturnCircleRevealStartTime >= 0 &&
    (wordsToDraw.length === 0 || returnRevealFinished)
  ) {
    gridReturnCircleRevealActive = false;
    gridReturnCircleRevealStartTime = -1;
  }
}

function drawFoundWordCircle(foundWord, alphaVal, wordIndex) {
  let revealProgress = 1;

  // Only outlines returning with the grid reform from the word's centre.
  // A newly found word stays at full size because the visitor already created
  // that outline by dragging across the letters.
  if (
    gridReturnCircleRevealActive &&
    gridReturnCircleRevealStartTime >= 0
  ) {
    let delayedElapsed =
      millis() -
      gridReturnCircleRevealStartTime -
      wordIndex * GRID_RETURN_CIRCLE_REVEAL_STAGGER;

    revealProgress = constrain(
      delayedElapsed / GRID_RETURN_CIRCLE_REVEAL_DURATION,
      0,
      1
    );
  }

  drawWordSelectionCapsule(
    foundWord.cells,
    alphaVal,
    revealProgress
  );
...
function drawWordSelectionCapsule(cells, alphaVal, revealProgress) {
  let points = [];
...
let topBottomPadding = stepY * 0.36;  
  let dashLength = 4.6;
  let dashGap = 4.2;
  let lineWeight = PAGMAR_INTERFACE_STROKE;
  ...
  let reveal = revealProgress === undefined
    ? 1
    : constrain(revealProgress, 0, 1);
  let easedReveal = easeInOutCubic(reveal);

  // During a return to the grid, the stored outline begins as a small circle
  // at the word's centre and extends outward into the full capsule.
  if (easedReveal < 1) {
    let centerX = boxX + boxW / 2;
    let startW = min(boxW, boxH);
    let animatedW = lerp(startW, boxW, easedReveal);
    boxX = centerX - animatedW / 2;
    boxW = animatedW;
  }
  ...
  ctx.globalAlpha =
    (alphaVal / 255) *
    PAGMAR_SELECTION_STROKE_ALPHA *
    lerp(0.08, 1, easedReveal);
  ctx.strokeStyle = "rgb(0,0,0)";
  // Keep the exact shared 0.5px interface weight. The reduced alpha prevents
  // the dashed capsule from appearing heavier than the dividers and buttons.
  ctx.lineWidth = lineWeight;
  ctx.setLineDash([dashLength, dashGap]);
  ctx.lineDashOffset = (1 - easedReveal) * 22;
  ctx.lineJoin = "round";
  ctx.lineCap = "round";
...
function setupHomeScreenDom() {
  ...
  homeScreenStyle.textContent = `
    ...
    .pagmar-home-scroll-guide.is-hidden {
      opacity: 0;
      pointer-events: none;
    }
    ...
    .pagmar-home-scroll-arrow::before {
      top: 0;
      animation: pagmar-home-scroll-invite 1.35s ease-in-out infinite;
    }

    .pagmar-home-scroll-arrow::after {
      top: 9px;
      animation: pagmar-home-scroll-invite 1.35s ease-in-out .13s infinite;
    }

    @keyframes pagmar-home-scroll-invite {
      0%, 100% {
        opacity: .18;
  ...
  homeScreenScrollGuide = document.createElement("div");
  homeScreenScrollGuide.className = "pagmar-home-scroll-guide";
  ...
  homeScreenScrollGuide.appendChild(arrow);
  ...
  function pauseHomeInteractionGuide() {
    if (!homeScreenScrollGuide) return;

    homeScreenScrollGuide.classList.add("is-hidden");

    if (homeInteractionGuideRestartTimer !== null) {
      clearTimeout(homeInteractionGuideRestartTimer);
    }

    homeInteractionGuideRestartTimer = setTimeout(function() {
      homeInteractionGuideRestartTimer = null;

      if (
        homePopupOpen &&
        homeScreenRoot &&
        homeScreenRoot.style.display !== "none" &&
        homeScreenScrollGuide
      ) {
  ...
  homeInteractionGuideActivityHandler = pauseHomeInteractionGuide;

  homeScreenRoot.addEventListener("scroll", function() {
    pauseHomeInteractionGuide();
  }, { passive: true });
...
function drawHomeHeaderOverlay(b, alphaVal) {
  ...
  strokeWeight(PAGMAR_INTERFACE_STROKE);
  noFill();
  rect(b.x, b.y, b.w, b.h);
...
function setupBorderScreenDom() {
  ...
  borderScreenStyle.textContent = `
    ...
    .pagmar-border-image-panel {
      ...
      transition:
        ...
        clip-path 1500ms cubic-bezier(.16,1,.3,1);
      pointer-events: none;
    ...
    .pagmar-border-svg-base {
      z-index: 1;
      opacity: 0.18;
    }

    .pagmar-border-svg-trail {
      z-index: 2;
      opacity: 1;
      clip-path: inset(0 0 100% 0);
      will-change: clip-path;
      filter:
        drop-shadow(1px 0 2px rgba(46,245,255,.18))
        drop-shadow(-1px 0 2px rgba(255,53,53,.14));
    }
    ...
    .pagmar-border-hotspot.is-auto-active::before {
      opacity: 1;
      transform: scale(1);
    }
  ...
  borderScreenBoard = board;

  const borderSvg = document.createElement("img");
  borderSvg.className = "pagmar-border-svg pagmar-border-svg-base";
  ...
  borderScreenTrailSvg = document.createElement("img");
  borderScreenTrailSvg.className =
    "pagmar-border-svg pagmar-border-svg-trail";
  borderScreenTrailSvg.src = "border.svg";
  borderScreenTrailSvg.alt = "";
  borderScreenTrailSvg.setAttribute("aria-hidden", "true");
  borderScreenTrailSvg.addEventListener("error", function() {
    console.log("MISSING FILE: border.svg (put it beside sketch.js)");
  });
  ...
  hebrewText.lang = "he";
  hebrewText.dir = "rtl";
  populateBorderAnimatedText(hebrewText, borderHebrewText);

  const arabicText = document.createElement("p");
  arabicText.className = "pagmar-border-text-layer pagmar-border-text-arabic";
  arabicText.lang = "ar";
  arabicText.dir = "rtl";
  populateBorderAnimatedText(arabicText, borderArabicText);

  borderScreenTextPanel.appendChild(hebrewText);
  borderScreenTextPanel.appendChild(arabicText);

  borderScreenImageHotspot = createBorderScreenHotspot(
    "pagmar-border-hotspot-one",
    "Border image point"
  );

  borderScreenTextHotspot = createBorderScreenHotspot(
    "pagmar-border-hotspot-two",
    "Border text point"
  );

  borderScreenSoundHotspot = createBorderScreenHotspot(
    "pagmar-border-hotspot-three",
    "Border sound point"
  );

  board.appendChild(borderSvg);
  board.appendChild(borderScreenTrailSvg);
  board.appendChild(borderScreenTextPanel);
  board.appendChild(borderScreenImageHotspot);
  board.appendChild(borderScreenTextHotspot);
  board.appendChild(borderScreenSoundHotspot);
...
function createBorderScreenHotspot(extraClass, label) {
  const marker = document.createElement("button");
  marker.type = "button";
  marker.className = "pagmar-border-hotspot " + extraClass;
...
function populateBorderAnimatedText(element, textValue) {
  if (!element) return;

  element.textContent = "";
  let characterIndex = 0;

  for (let character of Array.from(String(textValue))) {
    if (character === "\n") {
    ...
    span.className = "pagmar-border-character";
  ...
  element.dataset.characterCount = String(characterIndex);
}

function clampBorderPointProgress(value) {
  return Math.max(0, Math.min(1, value));
}

function getBorderHotspotViewportY(hotspot) {
  if (!borderScreenRoot || !hotspot) return Infinity;

  const rootRect = borderScreenRoot.getBoundingClientRect();
...
function getBorderTextPointScrollProgress() {
  if (!borderScreenRoot || !borderScreenTextHotspot) return 0;

  const viewportHeight = Math.max(1, borderScreenRoot.clientHeight);
  const pointY = getBorderHotspotViewportY(borderScreenTextHotspot);

  // Letters begin while the dot approaches the lower viewing area and
  // finish shortly after it reaches the center.
  const startY = viewportHeight * 0.88;
  const endY = viewportHeight * 0.46;

  return clampBorderPointProgress(
    (startY - pointY) / Math.max(1, startY - endY)
  );
}

function updateBorderTextCharacterProgress(progress) {
  if (!borderScreenTextPanel) return;

  borderTextScrollProgress =
    clampBorderPointProgress(progress);

  const layers = borderScreenTextPanel.querySelectorAll(
    ".pagmar-border-text-layer"
  );

  layers.forEach(function(layer) {
    const characters = layer.querySelectorAll(
      ".pagmar-border-character"
    );
    const count = Math.max(1, characters.length);

    characters.forEach(function(character, index) {
      // The start positions are staggered, but every individual letter
      // completes its own upward movement quickly.
      const staggerStart =
        count <= 1 ? 0 : (index / (count - 1)) * 0.82;
      const localProgress = clampBorderPointProgress(
        (borderTextScrollProgress - staggerStart) / 0.12
      );
      const eased =
      ...
      character.style.top =
        (30 * (1 - eased)).toFixed(2) + "px";
    });
  });

  const textIsActive = borderTextScrollProgress > 0.01;
  borderScreenTextRevealed = textIsActive;

  if (borderScreenTextHotspot) {
    borderScreenTextHotspot.classList.toggle(
      "is-auto-active",
      textIsActive
    );
  }
  ...
  if (textIsActive) {
    borderActivePointId = "point-text";
  }
}

function updateBorderAutomaticPoints() {
  if (!borderScreenRoot || !borderPopupOpen) return;

  updateBorderTextCharacterProgress(
    getBorderTextPointScrollProgress()
  );

  if (borderScreenImageHotspot) {
    borderScreenImageHotspot.classList.add(
      "is-auto-active"
    );
  }

  if (borderScreenSoundHotspot) {
    const viewportHeight = Math.max(
      1,
      borderScreenRoot.clientHeight
    );
    const soundPointY =
      getBorderHotspotViewportY(borderScreenSoundHotspot);
    const soundReached =
      soundPointY <= viewportHeight * 0.58;

    borderScreenSoundHotspot.classList.toggle(
      "is-auto-active",
      soundReached
    );

    if (soundReached && !borderScreenSoundTriggered) {
      borderScreenSoundTriggered = true;
      borderActivePointId = "point-sound";
      playBorderScreenSound();
    }
  }
}

function revealBorderScreenImages(restartAnimation = false) {
  if (!borderScreenImagePanel) return;

  if (restartAnimation) {
    borderScreenImagePanel.classList.remove("is-visible");

    // Force the browser to commit the closed slit before reopening it.
    void borderScreenImagePanel.offsetWidth;
  }

  borderScreenImageRevealed = true;
  borderActivePointId = "point-image";
...
function revealBorderScreenText() {
  updateBorderTextCharacterProgress(1);
}
...
function clampBorderTrailProgress(value) {
  return Math.max(0, Math.min(1, value));
}

function getBorderTrailScrollProgress() {
  if (!borderScreenRoot || !borderScreenBoard) return 0;

  const maxScroll = Math.max(
    1,
    borderScreenRoot.scrollHeight - borderScreenRoot.clientHeight
  );

  // The last interactive point is at 76.8089% of the border board.
  // Complete the trail when that point reaches approximately the middle
  // of the visible screen, rather than waiting for overscroll below it.
  const lastPointContentY =
    borderScreenBoard.offsetTop +
    borderScreenBoard.offsetHeight * 0.768089;

  const naturalEndScroll =
    lastPointContentY - borderScreenRoot.clientHeight * 0.52;

  const endScroll = Math.max(
    1,
    Math.min(maxScroll, naturalEndScroll)
  );

  return clampBorderTrailProgress(
    borderScreenRoot.scrollTop / endScroll
  );
}

function drawBorderTrailProgress(progress) {
  if (!borderScreenTrailSvg) return;

  const safeProgress = clampBorderTrailProgress(progress);

  // Keep the first point visible at rest. As the visitor scrolls, the exact
  // border SVG is uncovered from top to bottom like one connected trail.
  const visiblePercent = 2.4 + safeProgress * 97.6;
  const hiddenPercent = Math.max(0, 100 - visiblePercent);

  borderScreenTrailSvg.style.clipPath =
    "inset(0 0 " + hiddenPercent.toFixed(3) + "% 0)";
}

function animateBorderTrail() {
  borderTrailAnimationFrame = null;

  const difference =
    borderTrailTargetProgress - borderTrailProgress;

  if (Math.abs(difference) < 0.001) {
    borderTrailProgress = borderTrailTargetProgress;
    drawBorderTrailProgress(borderTrailProgress);
    return;
  }

  // Smoothly follow the finger/scroll without lagging far behind it.
  borderTrailProgress += difference * 0.24;
  drawBorderTrailProgress(borderTrailProgress);

  borderTrailAnimationFrame =
    requestAnimationFrame(animateBorderTrail);
}

function updateBorderTrailFromScroll() {
  borderTrailTargetProgress = getBorderTrailScrollProgress();

  if (borderTrailTargetProgress >= 0.997) {
    borderTrailTargetProgress = 1;
  }

  if (borderTrailAnimationFrame === null) {
    borderTrailAnimationFrame =
      requestAnimationFrame(animateBorderTrail);
  }
}

function resetBorderTrail() {
  if (borderTrailAnimationFrame !== null) {
    cancelAnimationFrame(borderTrailAnimationFrame);
    borderTrailAnimationFrame = null;
  }

  borderTrailProgress = 0;
  borderTrailTargetProgress = 0;
  drawBorderTrailProgress(0);
}

function resetBorderScreenDom() {
  borderScreenImageRevealed = false;
  borderScreenTextRevealed = false;
  borderScreenSoundTriggered = false;
  borderTextScrollProgress = 0;
  resetBorderTrail();
  ...
  if (borderScreenTextPanel) {
    updateBorderTextCharacterProgress(0);
  }

  [
    borderScreenImageHotspot,
    borderScreenTextHotspot,
    borderScreenSoundHotspot
  ].forEach(function(hotspot) {
    if (hotspot) {
      hotspot.classList.remove("is-auto-active");
    }
  });
...
function openBorderConstellationScreen() {
  ...
  borderInteractionGuideDismissed = false;
  borderInteractionGuideLastActivityAt = -Infinity;
...
function drawSharedFooter(alphaVal) {
  ...
  // One contextual action lives directly beside the current page name:
  // HINT on the grid, GRID inside a found-word screen.
...
function canShowFooterHintButton() {
  // Hints belong only to the main grid. Found-word screens show only GRID.
  if (!canUseGridHint() || animatingGridChange) return false;
...
function getNextUnfoundHintPair() {
  for (let hebrewWord of PAGMAR_HINT_HEBREW_ORDER) {
    let pair = coreWordPairs.find(function(candidate) {
      return candidate.hebrew === hebrewWord;
    });
...
function resetGridHint() {
  gridHintStage = 0;
  gridHintPair = null;
  gridHintFlashStartedAt = -Infinity;
}
...
function activateGridHint() {
  ...
  // The second press starts the first-letter flash. Every later press replays
  // that same flash, so the visitor is never locked out before finding the word.
  gridHintStage = 2;
  gridHintFlashStartedAt = getPagmarGuideClock();
}
...
function drawFooterActionButtons(
  eyeY,
  footerH,
  alphaVal,
  dark,
  ...
  const showGrid = canShowFooterGridButton();
  const showHint = !showGrid && canShowFooterHintButton();

  if (!showHint && !showGrid) return;

  const buttonW = PAGMAR_FOOTER_ACTION_WIDTH;
  const buttonH = PAGMAR_FOOTER_ACTION_HEIGHT;

  // Reserve one fixed-width page-name slot at the right edge. This prevents
  // longer words from shifting the action button between interfaces.
  const centerX =
    wordRightX -
    PAGMAR_FOOTER_PAGE_SLOT_WIDTH -
    PAGMAR_FOOTER_ACTION_GAP -
    buttonW / 2;
...
function getGridHintCellAlphaMultiplier(rowIndex, colIndex) {
  if (gridHintStage !== 2) return 1;

  let cells = getActiveGridHintCells();
  if (!cells || cells.length === 0) return 1;

  let firstCell = cells[0];
  if (rowIndex !== firstCell.row || colIndex !== firstCell.col) return 1;

  const elapsed = getPagmarGuideClock() - gridHintFlashStartedAt;
  const totalDuration = GRID_HINT_FLASH_CYCLE_MS * GRID_HINT_FLASH_CYCLES;

  // The flash plays a few slow, smooth opacity cycles and then stops at full
  // opacity. Pressing HINT again restarts these cycles on the same first letter.
  if (!Number.isFinite(elapsed) || elapsed < 0 || elapsed >= totalDuration) {
    return 1;
  }

  const phase = (elapsed % GRID_HINT_FLASH_CYCLE_MS) / GRID_HINT_FLASH_CYCLE_MS;
  const smoothFlash = 0.5 + 0.5 * Math.cos(phase * TWO_PI);
  return lerp(0.18, 1, smoothFlash);
}
...
function setupSharedDraggableLens() {
  ...
  sharedLensStyle.textContent = `
    #pagmar-shared-lens {
      ...
      -webkit-tap-highlight-color: transparent;
      transform: translate(-50%, -50%);
      box-shadow: inset 0 0 0 0.5px rgba(17,17,17,0.14);
    ...
    #pagmar-shared-lens.is-dragging {
      ...
      box-shadow: inset 0 0 0 0.5px rgba(17,17,17,0.28);
    }
  `;
...
function returnToMainGridFromNavigation(replayEntryAnimation = true) {
  ...
  if (replayEntryAnimation) {
    gridInteractionGuideDismissed = false;
...
function mouseWheel(event) {
  ...
  let horizontalDelta =
    event && typeof event.deltaX === "number" ? event.deltaX : 0;
...
function mousePressed() {
  ...
  if (belongingPopupOpen) {
    // The shared top-bar title is handled above. Dragging anywhere in the
    // content area moves the belonging gallery horizontally.
    if (
      mouseY > getHeaderHeight() &&
      mouseY < height - getFooterHeight()
    ) {
      beginBelongingHorizontalDrag(mouseX);
    }
...
function mouseDragged() {
  if (belongingPopupOpen) {
    updateBelongingHorizontalDrag(mouseX);
    return;
  }
...
function addCellUnderMouse() {
  ...
  if (currentSelection.length === 0) {
    ...
    gridInteractionGuideDismissed = true;
    updatePagmarInteractionGuide();
...
function updatePendingFoundWordAnimation() {
  if (!pendingFoundWordAnimation) return;

  let elapsed = millis() - pendingFoundWordAnimation.startedAt;
  if (elapsed < FOUND_WORD_SCREEN_OPEN_DELAY) return;

  let pending = pendingFoundWordAnimation;
  pendingFoundWordAnimation = null;

  preparePagmarWordScreen(pending.word);
  openFoundWordDestination(pending.word, pending.cells);
}
...
function openFoundWordDestination(word, cells) {
  ...
  if (isBorderWord(word)) {
    openBorderPopupFromCells(cells);
    return;
  }

  if (isClothesWord(word)) {
    openClothesPopupFromCells(cells);
    return;
  }
  ...
  if (isIdentityWord(word)) {
    if (
      typeof window !== "undefined" &&
      typeof window.openIdentityTicket === "function"
    ) {
      window.openIdentityTicket();
    }
    return;
  ...
  if (isReflectionWord(word)) {
    if (
      typeof window !== "undefined" &&
      typeof window.openReflectionTicket === "function"
    ) {
...
function addFoundWord(cells, word) {
  if (alreadyFound(cells) || pendingFoundWordAnimation) return;
...
function windowResized() {
  ...
  if (belongingPopupOpen) {
    let previousMax = max(1, belongingHorizontalMaxOffset);
    let normalizedPosition = constrain(
      belongingHorizontalTarget / previousMax,
      0,
      1
    );
    ...
    belongingHorizontalTarget =
      normalizedPosition * belongingHorizontalMaxOffset;
...
function openMemoryPopup() {
  ...
  memoryInteractionGuideDismissed = false;
  memoryInteractionGuideLastActivityAt = -Infinity;
...
function openBelongingDoorsScreen() {
  ...
  belongingInteractionGuideDismissed = false;
  belongingInteractionGuideLastActivityAt = -Infinity;
  belongingPopupOpenStartedAt = millis();
  belongingActiveDoorId = null;

  resetBelongingHorizontalScroll();
...
function resetBelongingHorizontalScroll() {
  belongingHorizontalOffset = 0;
  belongingHorizontalTarget = 0;
  belongingHorizontalMaxOffset = max(0, width * (BELONGING_PANEL_COUNT - 1));
  belongingHorizontalDragging = false;
  belongingHorizontalVelocity = 0;
...
function beginBelongingHorizontalDrag(pointerX) {
  markPagmarInteractionGuideActivity("belonging");
  belongingHorizontalDragging = true;
  belongingHorizontalDragStartX = pointerX;
  belongingHorizontalDragStartOffset = belongingHorizontalTarget;
  belongingHorizontalLastPointerX = pointerX;
  belongingHorizontalLastPointerTime = millis();
  belongingHorizontalVelocity = 0;
}

function updateBelongingHorizontalDrag(pointerX) {
  if (!belongingHorizontalDragging) return;

  markPagmarInteractionGuideActivity("belonging");
  let now = millis();
  let elapsed = max(1, now - belongingHorizontalLastPointerTime);
  let pointerDelta = pointerX - belongingHorizontalLastPointerX;

  belongingHorizontalTarget = constrain(
    belongingHorizontalDragStartOffset +
      (pointerX - belongingHorizontalDragStartX),
    0,
    belongingHorizontalMaxOffset
  );
  ...
  belongingHorizontalLastPointerX = pointerX;
  belongingHorizontalLastPointerTime = now;
}

function endBelongingHorizontalDrag() {
  if (!belongingHorizontalDragging) return;

  belongingHorizontalDragging = false;

  // Settle onto the nearest full-screen panel after the swipe.
  let projected =
    belongingHorizontalTarget + belongingHorizontalVelocity * 8;
  let panelWidth = max(1, width);
  let panelIndex = constrain(
    Math.round(projected / panelWidth),
    0,
    BELONGING_PANEL_COUNT - 1
  );
...
function getBelongingPoemPanels() {
  let arabicStanzas = belongingArabicText
    .trim()
    .split(/\n\s*\n/);
  let hebrewStanzas = belongingHebrewText
    .trim()
    .split(/\n\s*\n/);

  return [
    {
      arabic: [arabicStanzas[0], arabicStanzas[1]].filter(Boolean).join("\n\n"),
      hebrew: [hebrewStanzas[0], hebrewStanzas[1]].filter(Boolean).join("\n\n")
    },
    {
      arabic: [arabicStanzas[2], arabicStanzas[3]].filter(Boolean).join("\n\n"),
      hebrew: [hebrewStanzas[2], hebrewStanzas[3]].filter(Boolean).join("\n\n")
    },
    {
      arabic: arabicStanzas[4] || "",
      hebrew: hebrewStanzas[4] || ""
    }
  ];
}
...
function drawBelongingScreen() {
  background(238);
  updateBelongingHorizontalScroll();

  let headerH = getHeaderHeight();
  let footerH = getFooterHeight();
  let viewportTop = headerH;
  let viewportBottom = height - footerH;
  let viewportH = max(1, viewportBottom - viewportTop);
  let panelW = width;
  let offset = belongingHorizontalOffset;
  let poemPanels = getBelongingPoemPanels();

  let rawProgress = constrain(
    (millis() - belongingPopupOpenStartedAt) /
      belongingPopupOpenDuration,
    0,
    1
  );
...
function drawBelongingOverlayTextScreen(
  ...
  let arabicLines = String(arabicText || "").split("\n");
  let hebrewLines = String(hebrewText || "").split("\n");
  let lineCount = max(arabicLines.length, hebrewLines.length);

  let fontSize = constrain(
    min(w * 0.055, h / max(10, lineCount * 1.2)),
    17,
    42
  );

  let lineHeight = fontSize * 1.16;
  let blankHeight = fontSize * 0.72;
  let totalHeight = 0;

  for (let i = 0; i < lineCount; i++) {
    let arLine = arabicLines[i] || "";
    let heLine = hebrewLines[i] || "";
    totalHeight += arLine === "" && heLine === "" ? blankHeight : lineHeight;
  }
  ...
  for (let i = 0; i < lineCount; i++) {
    ...
    ctx.fillText(arLine, rightX + 2.2, currentY - 1.5);

    ctx.fillStyle = "rgba(46,245,255," + (0.98 * progress) + ")";
    ctx.fillText(heLine, rightX - 2.2, currentY + 2.6);
...
function syncReflectionTicketFrameWithMainSketch() {
  ...
  if (!doc.__pagmarInteractionGuideBound) {
    doc.__pagmarInteractionGuideBound = true;

    const sliderInputs = Array.from(doc.querySelectorAll('input[type="range"]'));
...
function openReflectionTicket() {
  reflectionInteractionGuideDismissed = false;
  reflectionInteractionGuideLastActivityAt = -Infinity;
